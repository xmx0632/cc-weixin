...
const CMD_SEND_PREFIX = "/send ";

// 发送文件的意图模式（匹配具体文件名）
const SEND_FILE_PATTERNS = [
  /发送[一下]?[个]?([^\s]+?\.(png|jpg|jpeg|gif|webp|pdf|md|txt|doc|docx|mp3|wav|mp4|mov|zip))\s*$/i,
  /发[一个]?([^\s]+?\.(png|jpg|jpeg|gif|webp|pdf|md|txt|doc|docx|mp3|wav|mp4|mov|zip))\s*$/i,
  /send\s+([^\s]+?\.(png|jpg|jpeg|gif|webp|pdf|md|txt|doc|docx|mp3|wav|mp4|mov|zip))\s*$/i,
  /把([^\s]+?\.(png|jpg|jpeg|gif|webp|pdf|md|txt|doc|docx|mp3|wav|mp4|mov|zip))发[给送]/i,
];

// 文件类型映射（用于模糊匹配"发送一个MP3文件"这类请求）
const FILE_TYPE_PATTERNS = [
  { pattern: /发送(?:一下|一个|个)?(?:图片|照片)[文件]?/i, ext: /\.(png|jpg|jpeg|gif|webp)$/i },
  { pattern: /发送(?:一下|一个|个)?(?:PNG|JPG|JPEG|GIF|WebP)[文件]?/i, ext: /\.(png|jpg|jpeg|gif|webp)$/i },
  { pattern: /发送(?:一下|一个|个)?(?:音频|音乐)[文件]?/i, ext: /\.(mp3|wav)$/i },
  { pattern: /发送(?:一下|一个|个)?(?:MP3|WAV)[文件]?/i, ext: /\.(mp3|wav)$/i },
  { pattern: /发送(?:一下|一个|个)?(?:视频)[文件]?/i, ext: /\.(mp4|mov)$/i },
  { pattern: /发送(?:一下|一个|个)?(?:MP4|MOV)[文件]?/i, ext: /\.(mp4|mov)$/i },
  { pattern: /发送(?:一下|一个|个)?(?:文档)[文件]?/i, ext: /\.(pdf|doc|docx)$/i },
  { pattern: /发送(?:一下|一个|个)?(?:PDF|Word|DOC|DOCX)[文件]?/i, ext: /\.(pdf|doc|docx)$/i },
  { pattern: /发送(?:一下|一个|个)?(?:压缩包)[文件]?/i, ext: /\.zip$/i },
  { pattern: /发送(?:一下|一个|个)?(?:ZIP)[文件]?/i, ext: /\.zip$/i },
];
 ...
 * @param {RegExp} extPattern - 扩展名正则
 * @returns {string|null} - 找到的文件路径或 null
 */
function findFileByExtension(dir, extPattern) {
  try {
    const files = readdirSync(dir);
    for (const file of files) {
      const filePath = join(dir, file);
      const stat = statSync(filePath);
      if (stat.isFile() && extPattern.test(file)) {
        return filePath;
      }
    }
  } catch (e) {
 ...
 */
function resolveFileFromIntent(text) {
  // 1. 先尝试精确匹配文件名
  for (const pattern of SEND_FILE_PATTERNS) {
  ...
  // 2. 尝试文件类型模糊匹配
  for (const typePattern of FILE_TYPE_PATTERNS) {
    const match = text.match(typePattern.pattern);
    if (match) {
      // 如果有明确的扩展名模式
      if (typePattern.ext) {
        const found = findFileByExtension(PROJECT_ROOT, typePattern.ext);
        if (found) return found;
      }
      // 动态匹配（如 "发送一个MP3文件" 中的 MP3）
      if (typePattern.ext === null && match[1]) {
        ...
        if (extMap[ext]) {
          const found = findFileByExtension(PROJECT_ROOT, extMap[ext]);
          if (found) return found;
        }
      }
    }
...
/sessions   - 查看历史会话列表
/filters    - 查看响应过滤器列表
/filter <name> - 启用/禁用指定过滤器
/send <file_path> - 发送文件（图片/视频/文档）
/help       - 显示帮助信息`,
      };
    ...
    default:
      ...
      // /send 命令需要异步处理，返回特殊标记
      if (trimmed.startsWith(CMD_SEND_PREFIX)) {
        const filePath = trimmed.slice(CMD_SEND_PREFIX.length).trim();
        if (!filePath) {
          return { handled: true, reply: "用法: /send <file_path>\n例如: /send ./demo.png" };
        }
        return { handled: true, sendFile: filePath };
      }

      // 自动检测"发送xxx文件"的意图（使用新的智能匹配）
      const resolvedFile = resolveFileFromIntent(trimmed);
      if (resolvedFile) {
        return { handled: true, sendFile: resolvedFile };
      }
