...
// 发送文件标记（Claude 返回此标记时触发文件发送）
const SEND_FILE_MARKER = /<!-- SEND_FILE:\s*([^\s>]+)\s*-->/g;
...
// 文件发送能力提示词（注入到系统提示中）
const FILE_SEND_PROMPT = `
## 文件发送能力

当用户要求发送文件（图片、音频、视频、文档等）时：
1. 使用 Glob 工具搜索项目中匹配的文件
2. 确认文件存在后，在回复末尾添加发送标记：\`<!-- SEND_FILE: 文件路径 -->\`
3. 然后简单告知用户"正在发送..."

示例：
- 用户："发送一个MP3文件给我"
- 你搜索 \`**/*.mp3\`，找到 \`./music/demo.mp3\`
- 回复："好的，正在发送 demo.mp3 <!-- SEND_FILE: ./music/demo.mp3 -->"

支持的文件类型：png, jpg, jpeg, gif, webp, pdf, md, txt, doc, docx, mp3, wav, mp4, mov, zip
`;
...
/**
 * 从 Claude 响应中提取文件发送标记
 * @param {string} response - Claude 的响应文本
 * @returns {{ text: string, files: string[] }} - 清理后的文本和文件路径列表
 */
function extractSendFileMarkers(response) {
  const files = [];
  const text = response.replace(SEND_FILE_MARKER, (match, filePath) => {
...
export async function askClaude(userText, userId = "default") {
  ...
  if (process.env.DEBUG_SESSION === "1") {
    console.log(`[Session] userId=${userId}, claudeSessionId=${session.claudeSessionId}, isFirstMessage=${isFirstMessage}`);
  }
  ...
  async function* messages() {
    ...
    // 如果有历史记录，将其作为上下文附加
    if (recentHistory.length > 0 && !isFirstMessage) {
      const historyContext = recentHistory.map(msg => {
        const role = msg.role === 'user' ? '用户' : '助手';
        return `[${role}]: ${msg.content}`;
      }).join('\n');

      enhancedText = `以下是之前的对话记录：
---
${historyContext}
---
当前用户消息：${userText}`;
...
${skillsContent}`;
  ...
  try {
    for await (const msg of query({
      ...
      options: {
        ...
        // 通过 systemPrompt preset 注入 skills 内容和文件发送能力
        systemPrompt: { type: "preset", append: `${skillsContent ? `# 可用技能\n\n${skillsContent}\n\n` : ''}${FILE_SEND_PROMPT}` },
        // 额外的权限设置
        settings: {
          permissions: {
            allow: [
  ...
  if (claudeSessionId && session.claudeSessionId !== claudeSessionId) {
    ...
    if (process.env.DEBUG_SESSION === "1") {
      console.log(`[Session] 更新 claudeSessionId: ${claudeSessionId}`);
    }
  }
  ...
  // 提取文件发送标记
  const { text, files } = extractSendFileMarkers(filterResult.filtered);
  ...
  return { text: text || "（Claude 无回复）", files };
}
