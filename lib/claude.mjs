...
export async function askClaude(userText, userId = "default") {
  ...
  // 检查是否是第一条消息（没有 claudeSessionId）
  const isFirstMessage = !session.claudeSessionId;

  // 构建消息生成器（只发送当前消息，SDK 通过 session_id 维护上下文）
  async function* messages() {
    // 如果是第一条消息且有 skills 内容，注入 skills
...
${skillsContent}`;
      ...
      // 只发送当前消息，SDK 通过 session_id 自动维护上下文
      yield {
        type: "user",
        session_id: session.claudeSessionId || "",
        parent_tool_use_id: null,
  ...
  // 应用响应过滤器（清理 Sources、工具输出等）
  const currentHistory = getFormattedHistory(userId);
...
/help       - 显示帮助信息`,
    ...
    default:
      ...
      if (trimmed.startsWith("/filter ")) {
        const filterName = trimmed.slice(8).trim();
        if (!filterName) {
