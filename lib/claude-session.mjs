...
export function getOrCreateSession(userId) {
  ...
  if (process.env.DEBUG_SESSION === "1") {
    console.log(`[Session] getOrCreateSession: userId=${userId}, isStartup=${isStartup}, session=${session ? session.id : 'null'}, claudeSessionId=${session?.claudeSessionId || 'null'}`);
  }
  ...
  if (isStartup && session && session.claudeSessionId) {
    ...
    isStartup = false;
    if (process.env.DEBUG_SESSION === "1") {
      console.log(`[Session] 启动时清除 claudeSessionId`);
    }
  }
  ...
  if (!session) {
    ...
    isStartup = false;
    if (process.env.DEBUG_SESSION === "1") {
      console.log(`[Session] 创建新会话: ${session.id}`);
    }
  }
  return session;
}
